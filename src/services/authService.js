...
export async function signup(name, email, password, roleType, phone) {
  try {
    ...
    console.error('API Error (signup):', err.response?.data || err.message);
...
export async function login(email, password) {
  try {
    ...
    const message = err.response?.data?.message || err.message;
    console.error('API Error (login):', err.response?.data || err.message);
    throw new Error(message);
  }
}
...
export async function adminLogin(email, password) {
  try {
    ...
    const message = err.response?.data?.message || err.message;
    console.error('API Error (adminLogin):', err.response?.data || err.message);
    throw new Error(message);
  }
}
...
export async function getMe() {
  try {
    ...
    console.error('API Error (getMe):', err.response?.data || err.message);
    throw err;
  }
}
...
export async function forgotPassword(email) {
  try {
    ...
    const message = err.response?.data?.message || err.message;
    console.error('API Error (forgotPassword):', err.response?.data || err.message);
    throw new Error(message);
  }
}
...
export async function verifyResetOtp(email, otp, newPassword) {
  try {
    ...
    const message = err.response?.data?.message || err.message;
    console.error('API Error (verifyResetOtp):', err.response?.data || err.message);
    throw new Error(message);
  }
}
